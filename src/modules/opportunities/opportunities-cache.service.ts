...
/**
 * Firsat listesi (GET /opportunities) icin TTL'siz Redis cache -
 * accounts-cache.service.ts ile ayni desen: ayni ioredis client'i paylasir,
 * ayri bir anahtar namespace'i kullanir (tenant:{id}:opportunities:list:*).
 ...
 */
@Injectable()
export class OpportunitiesCacheService {
  constructor(@Inject(REDIS_CLIENT) private readonly redis: Redis) {}

  private buildKey(tenantId: string, params: unknown): string {
    return `tenant:${tenantId}:opportunities:list:${hashParams(params)}`;
  }

  async get(params: unknown): Promise<PagedResult<Opportunity> | null> {
    const { tenantId } = TenantContext.getOrThrow();
    const raw = await this.redis.get(this.buildKey(tenantId, params));
    if (!raw) {
      return null;
    }
    return JSON.parse(raw) as PagedResult<Opportunity>;
  }

  async set(params: unknown, result: PagedResult<Opportunity>): Promise<void> {
    const { tenantId } = TenantContext.getOrThrow();
  ...
  async invalidate(): Promise<void> {
    const { tenantId } = TenantContext.getOrThrow();
    const pattern = `tenant:${tenantId}:opportunities:list:*`;
